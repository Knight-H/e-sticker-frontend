...
export let EnhancedLocationFields = withRouter(
  withFormik({
    ...
    handleSubmit: (values, { props }) => {
      ...
      const moreUserInfo = Object.fromEntries(
      ...
      auth.onAuthStateChanged((userCredential) => {
        ...
        axiosInst
          .put(`customers/${uid}`, customerSchemaInfo, {
            headers: {
        ...
        <div className={styles.row}>
          ...
          <div className={styles.selectBox}>
            <div className={styles.selectBoxCurrent} tabIndex="0">
              <div className={`${styles.selectBoxValue}`}>
                <Field
                  ...
                  id="0"
                  value="ok"
                  checked={`${values.accountState}` === `ok` ? true : false}
                  onChange={(e) => handleChangeDropDawn(e.target)}
                />
                <p
                  className={`${styles.selectBoxInputText}  ${styles.comboBoxStatusGreen}`}
                >
                  ปกติ
                </p>
              </div>
              <div className={styles.selectBoxValue}>
                <Field
                  ...
                  id="1"
                  value="notOk"
                  checked={`${values.accountState}` === `notOk` ? true : false}
                  onChange={(e) => handleChangeDropDawn(e.target)}
                />
                <p
                  className={`${styles.selectBoxInputText}  ${styles.comboBoxStatusRed}`}
                >
                  แบน
                </p>
                <IconArrow />
            ...
            <ul className={styles.selectBoxList}>
              <li>
                <label
                  className={`${styles.selectBoxOption} ${styles.comboBoxStatusGreen}`}
                  for="0"
                >
                  ปกติ
                </label>
              ...
              <li>
                <label
                  className={`${styles.selectBoxOption} ${styles.comboBoxStatusRed}`}
                  for="1"
                >
                  แบน
                </label>
...
let MemberComponent = (props) => {
  ...
  useEffect(() => {
    ...
    axiosInst
      .get(`customers/${urlParams.user_id}`, {
        headers: {
          Authorization: "Basic ZGlnaXRhbHdpc2g6SzZDd2N3dkF6QVNDRGZWNg==",
        },
      })
