...
const ModalShipping = ({ values, setFieldValue }) => {
  useEffect(() => {
    ...
    console.log("values.materialID", values.materialID);
    if (values.materialID || values.materialID === 0) {
      if (values.material[values.materialID]) {
        setFieldValue(
          `${values.modalMaterial}Kind`,
          values.material[values.materialID].name,
          false
        );
        setFieldValue(
          `${values.modalMaterial}Description`,
          values.material[values.materialID].description,
          false
        );
        setFieldValue(
  ...
  const editOptionMaterial = () => {
    ...
    if (values[`${values.modalMaterial}File`]) {
      if (typeof values[`${values.modalMaterial}File`] !== "object") {
        values.material[values.materialID].name =
          values[`${values.modalMaterial}Kind`];
        values.material[values.materialID].description =
          values[`${values.modalMaterial}Description`];
        values.material[values.materialID].imgUrl =
    ...
    >
      <div className={styles.modalContent}>
        ...
        <div className={styles.rowInModal}>
          <Field
            name={`${values.modalMaterial}Kind`}
