...
const ModalShipping = ({ values, setFieldValue }) => {
    ...
    useEffect(() => {
        if (values.shapeID || values.shapeID === 0) {
            setFieldValue(`${values.modalShape}Kind`, values.shape[values.shapeID].name, false);
            setFieldValue(`${values.modalShape}File`, values.shape[values.shapeID].imgUrl, false);
        } else {
    ...
    const editOptionShape = () => {
        ...
        if (values[`${values.modalShape}File`]) {
            if (typeof values[`${values.modalShape}File`] !== 'object') {
                ...
                values.shape[values.shapeID].imgUrl = values[`${values.modalShape}File`]
    ...
    return (
        <div className={styles.modal} style={{ display: values.modalShape ? "block" : "none" }}>
            <div className={styles.modalContent}>
                ...
                <div className={styles.rowInModal}>
                    <Field name={`${values.modalShape}Kind`} className={styles.text} placeholder="รูปแบบ" />
                </div>
                <div className={styles.rowInModal}>
                    <input type="file" id="file" onChange={(e) => handleChangeImgShape(e)} />
                ...
                <div className={`${styles.floatRight} ${styles.rowInModal}`}>
                    {values.shapeID || values.shapeID === 0 ?
                        <button type="button" className={styles.removeOption} onClick={() => removeOptionShape()}>ลบ</button> :
                        null}
                    <button type="button" className={styles.addOption} onClick={() => values.shapeID || values.shapeID === 0 ? editOptionShape() : addOptionShape()}>บันทึก</button>
                </div>
            </div>
        </div>
    )
