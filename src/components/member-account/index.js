...
import { ReactComponent as IconPhone } from "../home-member/icon-phone.svg";
import { ReactComponent as IconUser } from "../home-member/icon-user.svg";
import { ReactComponent as IconLogout } from "../home-member/icon-logout.svg";
...
export const EnhancedLoginCredentialsComponent = withFormik({
  ...
  handleSubmit: async (values, { props }) => {
    ...
    try {
      freshCredential = Firebase.auth.EmailAuthProvider.credential(
    ...
    try {
      ...
      return props.setUpdateStatusText(
        i18.account_password_change_failed_general,
        e
      );
    }
  },
  ...
  validate: (values) => {
    ...
    Object.entries(values)
      .filter(([fieldName, _fieldValue]) => {
        return !["line_channel", "line_token", "loading", "isAdmin"].includes(
          fieldName
        );
      })
        ...
        .then((res) => {
          ...
          if (!documentKey) {
            ...
            axiosInst
              ...
              .then((res) => {
                props.setUpdateStatusText(
                  i18.account_information_update_success
                );
                setFieldValue("loading", false, false);
              })
              .catch((reason) => {
                props.setUpdateStatusText(
                  i18.account_information_update_failed_general
                );
                setFieldValue("loading", false, false);
              });
          } else {
            // Otherwise update
            axiosInst
              .put(api.customers + `/${documentKey}`, customerSchemaInfo, {
                headers: {
                  Authorization:
                    "Basic ZGlnaXRhbHdpc2g6SzZDd2N3dkF6QVNDRGZWNg==",
                },
...
export const Intermediate = ({ setName }) => {
  ...
  return (
    <>
      {(() => {
        if (
          updateStatusText === i18.account_information_update_success ||
          updateStatusText === i18.account_password_change_success
        ) {
          return (
            <p className={styles.accountCreateSuccess}>{updateStatusText}</p>
          );
        } else if (updateStatusText !== null) {
          return (
            <p className={styles.accountCreateFailed}>{updateStatusText}</p>
          );
        }
        return <p>　</p>;
      })()}
      ...
      <div className={styles.flexWrapper}>
        {userInfo.line_channel ? (
          <div className={styles.loginCredentialsNoneDisplay}></div>
        ) : (
          <EnhancedLoginCredentialsComponent
            emailDisabled={true}
            userInfo={userInfo}
            setUserInfo={setUserInfo}
            setUpdateStatusText={setUpdateStatusText}
          />
        )}
